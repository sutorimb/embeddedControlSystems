  ...
  if (pcm_nRecTriggerMode) {
...

    {
      switch (pcm_nTrgVarSize)
      {
       ...
       case 2:
        pcm_pCompareFunc = pcm_bTrgVarSigned ? FMSTR_Compare16S :
          FMSTR_Compare16U;
        break;

       case 4:
        pcm_pCompareFunc = pcm_bTrgVarSigned ? FMSTR_Compare32S :
          FMSTR_Compare32U;
        break;
...
FMSTR_BOOL FMSTR_IsInRecBuffer(FMSTR_ADDR dwAddr, FMSTR_SIZE8 nSize)
{
  ...
  if (dwAddr >= pcm_nRecBuffAddr) {
    bRet = (FMSTR_BOOL)((dwAddr + nSize) <= (pcm_nRecBuffAddr +
      FMSTR_GetRecBuffSize()) ? FMSTR_TRUE : FMSTR_FALSE);
  }
