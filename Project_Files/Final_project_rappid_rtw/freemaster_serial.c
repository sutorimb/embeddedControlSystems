    ...
    if (pcm_wFlags.flg.bTxActive || pcm_wFlags.flg.bMqxReadyToSend) {
      while (1) {
        ...
        if (pcm_wFlags.flg.bMqxReadyToSend) {
          if (fwrite(&TxChar, 1, 1, devfd))/* character was successfully send, ready to get next character */
            pcm_wFlags.flg.bMqxReadyToSend = 0;
          else                         /* character write failed, needs to be send next call */
            break;
        }

        /* is ready to get next character? */
        if (FMSTR_Tx((FMSTR_U8*)&TxChar))/* FreeMASTER packet is sent, exit loop */
          break;
  ...
  )
{
  ...
  else if(event_type == USB_APP_ENUM_COMPLETE)
  {
    pcm_wFlags.flg.bUsbCdcStartApp = 1U;
          ...
          (pcm_wFlags.flg.bUsbCdcStartTrans))
  {
    ...
    if (pcm_wFlags.flg.bUsbCdcStartApp) {
      ...
      if (!pcm_wFlags.flg.bTxActive) {
        ...
        if (index == BytesToBeCopied)  /* receive next packet only when are not received all bytes of packet */
          ...
          (pcm_wFlags.flg.bUsbCdcStartTrans))
  {
    if ((pcm_wFlags.flg.bUsbCdcStartApp) && (pcm_wFlags.flg.bTxActive))/* Finalize USB packet to transmit and send packet */
      FMSTR_SendUsbPacket();
    else                               /* Previous Send is complete. Queue next receive */
      (void)USB_Class_CDC_Interface_DIC_Recv_Data(FMSTR_USB_CDC_ID, NULL, 0);
  }
...
void FMSTR_SendResponse(FMSTR_BPTR pResponse, FMSTR_SIZE8 nLength)
{
  ...
  pResponse = FMSTR_ValueToBuffer8(pResponse, (FMSTR_U8) (((FMSTR_U16)~(chSum))
    + 1U));
  ...
  if ((pcm_wFlags.flg.bUsbCdcStartApp) && (pcm_wFlags.flg.bTxActive) &&
      (pcm_wFlags.flg.bUsbCdcStartTrans))/* Finalize USB packet to transmit and send packet */
    FMSTR_SendUsbPacket();
  else                                 /* Previous Send is complete. Queue next receive */
    (void)USB_Class_CDC_Interface_DIC_Recv_Data(FMSTR_USB_CDC_ID, NULL, 0);
    ...
    *pcm_pRxBuff++ = nRxChar;
    ...
    if (!((~nRxChar) & FMSTR_FASTCMD)) {
      ...
      pcm_nRxTodo = (FMSTR_SIZE8)
        (((((FMSTR_SIZE8)nRxChar) & FMSTR_FASTCMD_DATALEN_MASK) >>
          FMSTR_FASTCMD_DATALEN_SHIFT) + 1U);
    }
