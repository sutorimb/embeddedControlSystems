...
/* make inline */
inline FMSTR_BOOL FMSTR_CheckMemSpace(FMSTR_ADDR nAddrUser, FMSTR_SIZE8
  nSizeUser,
...
static FMSTR_BOOL FMSTR_CheckMemSpace(FMSTR_ADDR nAddrUser, FMSTR_SIZE8
  nSizeUser,
  FMSTR_ADDR nAddrSafe, FMSTR_SIZE wSizeSafe);
static
#endif
  ...
  FMSTR_ADDR nAddrSafe, FMSTR_SIZE wSizeSafe)
{
  ...
  if (nAddrUser >= nAddrSafe) {
    bRet = (FMSTR_BOOL)
      (((nAddrUser + nSizeUser) <= (nAddrSafe + wSizeSafe)) ? FMSTR_TRUE :
       FMSTR_FALSE);
  }
 ...
 *
 ******************************************************************************/
FMSTR_BOOL FMSTR_CheckTsaSpace(FMSTR_ADDR dwAddr, FMSTR_SIZE8 nSize, FMSTR_BOOL
  bWriteAccess)
{
  ...
  for (nTableIndex=0U; (pte=FMSTR_TsaGetTable(nTableIndex, &cnt)) != NULL;
       nTableIndex++) {
    ...
    for (i=0U; i<cnt; i++) {
      ...
      if ((nInfo & FMSTR_TSA_INFO_VAR_FLAG) && (!bWriteAccess || (nInfo &
            FMSTR_TSA_INFO_RWV_FLAG))) {
        /* need to take the larger of the two in union (will be optimized by compiler anyway) */
        /*lint -e{506,774} condition always true/false */
        if (sizeof(pte->addr.p) < sizeof(pte->addr.n)) {
          if (FMSTR_CheckMemSpace(dwAddr, nSize, (FMSTR_ADDR) pte->addr.n,
                                  (FMSTR_SIZE) (nInfo >> 2))) {
            return FMSTR_TRUE;         /* access granted! */
          }
        } else {
          /*lint -e{923} casting pointer to long (on some architectures) */
          if (FMSTR_CheckMemSpace(dwAddr, nSize, (FMSTR_ADDR) pte->addr.p,
                                  (FMSTR_SIZE) (nInfo >> 2))) {
            return FMSTR_TRUE;         /* access granted! */
          }
        }
